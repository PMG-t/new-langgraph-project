...
class SPICalculationCodeEditorTool(BaseAgentTool):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            name = SPI_CALCULATION_CODE_EDITOR_TOOL,
            description = """Edit an existing Jupyter notebook related to Standardized Precipitation Index (SPI) by adding new code lines. It must be provided as an absolute path in local filesystem.
            Use this tool when user asks for an help in SPI calculation and wants to edit the notebook by adding something new.""",
            args_schema = SPICalculationCodeEditorTool.InputSchema,
            **kwargs
        )
